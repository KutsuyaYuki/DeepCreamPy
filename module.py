...
def conv_nn(input, dims1, dims2, size1, size2, k_size = 3):
    ...
    L1 = layers.conv2d(pp, dims1, [k_size, k_size], strides=[1, 1], padding='VALID', activation=None)
    L1 = tf.nn.elu(L1)

    pp = tf.pad(tensor=L1, paddings=[[0, 0], [1, 1], [1, 1], [0, 0]], mode="REFLECT")
    L2 = layers.conv2d(pp, dims2, [k_size, k_size], strides=[1, 1], padding='VALID', activation=None)
    L2 = tf.nn.elu(L2)
...
def encoder(input, reuse, name):
    with tf.compat.v1.variable_scope(name):
        ...
        CL1 = layers.conv2d(p, 32, [5, 5], strides=[1, 1], padding='VALID', activation=None)
        CL1 = tf.nn.elu(CL1)  # 256 256 32

        p = tf.pad(tensor=CL1, paddings=[[0, 0], [1, 1], [1, 1], [0, 0]], mode="REFLECT")
        CL2 = layers.conv2d(p, 64, [3, 3], strides=[2, 2], padding='VALID', activation=None)
        CL2 = tf.nn.elu(CL2)  # 128 128 64

        p = tf.pad(tensor=CL2, paddings=[[0, 0], [1, 1], [1, 1], [0, 0]], mode="REFLECT")
        CL3 = layers.conv2d(p, 64, [3, 3], strides=[1, 1], padding='VALID', activation=None)
        CL3 = tf.nn.elu(CL3)  # 128 128 64

        p = tf.pad(tensor=CL3, paddings=[[0, 0], [1, 1], [1, 1], [0, 0]], mode="REFLECT")
        CL4 = layers.conv2d(p, 128, [3, 3], strides=[2, 2], padding='VALID', activation=None)
        CL4 = tf.nn.elu(CL4)  # 64 64 128

        p = tf.pad(tensor=CL4, paddings=[[0, 0], [1, 1], [1, 1], [0, 0]], mode="REFLECT")
        CL5 = layers.conv2d(p, 128, [3, 3], strides=[1, 1], padding='VALID', activation=None)
        CL5 = tf.nn.elu(CL5)  # 64 64 128

        p = tf.pad(tensor=CL5, paddings=[[0, 0], [1, 1], [1, 1], [0, 0]], mode="REFLECT")
        CL6 = layers.conv2d(p, 256, [3, 3], strides=[2, 2], padding='VALID', activation=None)
        CL6 = tf.nn.elu(CL6)  # 32 32 128

        p = tf.pad(tensor=CL6, paddings=[[0, 0], [2, 2], [2, 2], [0, 0]], mode="REFLECT")
        DCL1 = layers.conv2d(p, 256, [3, 3], dilation_rate=2, strides=[1, 1], padding='VALID', activation=None)
        DCL1 = tf.nn.elu(DCL1)
        p = tf.pad(tensor=DCL1, paddings=[[0, 0], [4, 4], [4, 4], [0, 0]], mode="REFLECT")
        DCL2 = layers.conv2d(p, 256, [3, 3], dilation_rate=4, strides=[1, 1], padding='VALID', activation=None)
        DCL2 = tf.nn.elu(DCL2)
        p = tf.pad(tensor=DCL2, paddings=[[0, 0], [8, 8], [8, 8], [0, 0]], mode="REFLECT")
        DCL3 = layers.conv2d(p, 256, [3, 3], dilation_rate=8, strides=[1, 1], padding='VALID', activation=None)
        DCL3 = tf.nn.elu(DCL3)
        p = tf.pad(tensor=DCL3, paddings=[[0, 0], [16, 16], [16, 16], [0, 0]], mode="REFLECT")
        DCL4 = layers.conv2d(p, 256, [3, 3], dilation_rate=16, strides=[1, 1], padding='VALID', activation=None)
...
def decoder(input, size1, size2, reuse, name):
    with tf.compat.v1.variable_scope(name):
        ...
        LL2 = layers.conv2d(DL4, 3, [3, 3], strides=[1, 1], padding='SAME', activation=None)  # 256 256 3
...
def discriminator_G(input, reuse, name):
    with tf.compat.v1.variable_scope(name):
        ...
        p = tf.pad(tensor=input, paddings=[[0, 0], [2, 2], [2, 2], [0, 0]], mode="REFLECT")
        L1 = layers.conv2d(p, 64, [5, 5], strides=2, padding='VALID', activation=None)
        #L1 = instance_norm(L1, 'di1')
        L1 = tf.nn.leaky_relu(L1)

        p = tf.pad(tensor=L1, paddings=[[0, 0], [2, 2], [2, 2], [0, 0]], mode="REFLECT")
        L2 = layers.conv2d(p, 128, [5, 5], strides=2, padding='VALID', activation=None)
        #L2 = instance_norm(L2, 'di2')
        L2 = tf.nn.leaky_relu(L2)

        p = tf.pad(tensor=L2, paddings=[[0, 0], [2, 2], [2, 2], [0, 0]], mode="REFLECT")
        L3 = layers.conv2d(p, 256, [5, 5], strides=2, padding='VALID', activation=None)
        #L3 = instance_norm(L3, 'di3')
        L3 = tf.nn.leaky_relu(L3)

        p = tf.pad(tensor=L3, paddings=[[0, 0], [2, 2], [2, 2], [0, 0]], mode="REFLECT")
        L4 = layers.conv2d(p, 256, [5, 5], strides=2, padding='VALID', activation=None)
...
def discriminator_L(input, reuse, name):
    with tf.compat.v1.variable_scope(name):
        ...
        p = tf.pad(tensor=L3, paddings=[[0, 0], [2, 2], [2, 2], [0, 0]], mode="REFLECT")
        L4 = layers.conv2d(p, 512, [5, 5], strides=2, padding='VALID', activation=None)
...
def contextual_block(bg_in, fg_in, mask, k_size, lamda, name, stride=1):
    with tf.compat.v1.variable_scope(name):
        ...
        ACL2 = layers.conv2d(con1, dims, [1, 1], strides=[1, 1], padding='VALID', activation=None, name='ML')
        ACL2 = tf.nn.elu(ACL2)
